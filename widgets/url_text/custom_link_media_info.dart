...
import 'package:racfmn/model/link_media_info.dart';
import 'package:racfmn/ui/theme/theme.dart';
import 'package:racfmn/widgets/url_text/link_preview.dart';
import 'package:http/http.dart' as http;

class CustomLinkMediaInfo extends StatelessWidget {
  const CustomLinkMediaInfo({Key? key, required this.url, required this.text})
      : super(key: key);
  final String url;
  final String text;
  ...
  String getUrl() {
    ...
    if (_matches.isNotEmpty) {
      return _matches.first.group(0) as String;
  ...
  Future<Either<Exception, LinkMediaInfo>> fetchLinkMediaInfoFromApi(
    ...
    try {
      var response = await http.Client()
          .get(Uri.tryParse('https://noembed.com/embed?url=' + url) as Uri)
...
var map = await pref.getLinkMediaInfo(url);
  ...
  Widget build(BuildContext context) {
    ...
    /// It is seen that `flutter_link_preview`
    /// package is unable to fetch youtube metadata
                          ...
                          if (model.providerUrl != null)
                            Row(
                              ...
                              children: [
                                Expanded(
                                    child: Text(
                                  Uri.tryParse(model.providerUrl as String)!
                                      .authority,
                                  maxLines: 1,
                                  overflow: TextOverflow.ellipsis,
