...
class LinkPreview extends StatelessWidget {
  const LinkPreview({Key? key, required this.url, required this.text})
      : super(key: key);
  final String url;
  final String text;

  /// Extract the url from text
  /// If text contains multiple weburl then only
  /// first url will be returned to fetch the url meta
  String getUrl() {
    if (text == null) {
  ...
  Widget build(BuildContext context) {
    ...
    if (uri == null) {
      ...
      /// `flutter_link_preview` package is unable to
      /// fetch firebase dynamic link meta data
      return SizedBox.shrink();
    }
